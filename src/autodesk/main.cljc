(ns autodesk.main
  "Kotodama WASM entrypoint for the Autodesk clean-room actor (L5) — Clojure port.
  ...
  facts (`autodesk.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "autodesk")
(def tier "L5")
...
(def entity-specs
  [{:entity "Hub"      :plural "hubs"      :id-prefix "autodesk_hub"
    :fields [:name :region]                             :required [:name :region]
    :coerce {}                       :refs {}}
   {:entity "Project"  :plural "projects"  :id-prefix "autodesk_pro"
    :fields [:name]                                     :required [:name]
    :coerce {}                       :refs {}}
   {:entity "Folder"   :plural "folders"   :id-prefix "autodesk_fol"
    :fields [:name :displayName :createTime :createUserId :createUserName :lastModifiedTime :lastModifiedUserId :lastModifiedUserName :lastModifiedTimeRollup :objectCount :hidden]
    :required [:name :displayName]
    :coerce {:objectCount :int :hidden :bool} :refs {}}
   {:entity "Item"     :plural "items"     :id-prefix "autodesk_ite"
    :fields [:displayName :createTime :createUserId :createUserName :lastModifiedTime :lastModifiedUserId :lastModifiedUserName :hidden :reserved :reservedTime :reservedUserId :reservedUserName]
    :required [:displayName :createTime]
    :coerce {:hidden :bool :reserved :bool} :refs {}}
   {:entity "Version"  :plural "versions"  :id-prefix "autodesk_ver"
    :fields [:name :displayName :createTime :createUserId :createUserName :lastModifiedTime :lastModifiedUserId :lastModifiedUserName :versionNumber :storageSize :fileType]
    :required [:name :displayName]
    :coerce {:versionNumber :int :storageSize :int} :refs {}}
   {:entity "Command"  :plural "commands"  :id-prefix "autodesk_com"
...
(defn emit-facts
  "EAVT facts for one record: {\"autodesk.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "autodesk-compat" :tier tier :entities entities} 200])
...
(defn start! [] :autodesk-compat/ready)
